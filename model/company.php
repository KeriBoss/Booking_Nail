...
class Company extends Database{
     ...
     * 
     */
    function insert($name, $img, $hotline, $address,$time_start,$time_end, $status, $password, $day_start, $day_end){
        $sql = parent::$connection->prepare("INSERT INTO `company`(`company_name`, `img_company`, `hotline`, `address`, `time_start`, `time_end`, `status`, `password`, `day_start`, `day_end`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $sql->bind_param('ssssssisss', $name, $img, $hotline, $address, $time_start,$time_end, $status, $password, $day_start, $day_end);
        return $sql->execute();
    }
    /**
     * 
     ...
     * 
     */
    function getFullInfoCompany($company_id){
        $sql = parent::$connection->prepare("SELECT * FROM company WHERE company.`hotline` = ?");
        $sql->bind_param('s', $phone);
        return parent::select($sql);
    }
    ...
    function getStaffByCompanyId($company_id){
        $sql = parent::$connection->prepare("SELECT *, staff.id as staff_id FROM staff INNER JOIN services ON staff.service_id = services.id WHERE staff.`company_id` = ?");
        $sql->bind_param('i', $company_id);
        return parent::select($sql);
    }
    /**
