...
class Staff extends Database{
    ...
    function getAllStaff(){
        $sql = parent::$connection->prepare("SELECT *, staff.id as staff_id FROM staff INNER JOIN company ON staff.company_id = company.id INNER JOIN services ON services.id = staff.service_id ORDER BY staff.`id` DESC");
        return parent::select($sql);
    }
    /**
     * 
     */
    function insert($username, $avatar, $phone, $email, $address, $birth, $gender, $company_id, $service_id){
        $sql = parent::$connection->prepare("INSERT INTO `staff`(`user_name`, `avatar`, `phone`, `email`, `address`, `birth`, `gender`, `company_id`, `service_id`, `create_at`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, current_date)");
        $sql->bind_param('sssssssii', $username, $avatar, $phone, $email, $address, $birth, $gender, $company_id, $service_id);
        return $sql->execute();
    }
    /**
     * 
    ...
    function getStaffById($id){
        $sql = parent::$connection->prepare("SELECT *, staff.id as staff_id, services.id as service_id FROM staff INNER JOIN company ON staff.company_id = company.id INNER JOIN services ON services.id = staff.service_id WHERE staff.`id` = ?");
        $sql->bind_param('i', $id);
        return parent::select($sql);
    }
    /**
